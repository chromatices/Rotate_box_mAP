...
class _BaseAnnoComponents:
    ...
    def _validate_keys(self, info):
        ...
        if len(validate_elements) != sum(validate_elements):
            indices = [i for i, is_valid in enumerate(
                validate_elements) if is_valid is False]
...
class DetectionFile(_BaseAnnoComponents):
    ...
    def __init__(self, files_info: Dict) -> None:
        ...
        """
        self.REQUIRED_KEYS = ["filepath",
                              "image_width", "image_height", "objects"]
    ...
    def dump(self) -> None:
        ...
        print("\tnumber of objects:\t{}".format(len(self)))
...
class DetectionAnnotations(_BaseAnnoComponents):
    ...
    def __init__(self, anno_info: List):
        ...
        self.FILES = self._error_correction(self.FILES)
    ...
    def dump(self) -> None:
        ...
        """
        print("\tnumber of files:\t{}".format(len(self)))
        [FILE.dump() for FILE in self.FILES]

    def _error_correction(self, files: List[DetectionFile]) -> [List[DetectionFile]]:
        """
        ...
        overlab_dict = self._make_overlab_dict(filepath_list)
        for element in tqdm(unique_value, desc="Merge Duplicated Images Files..."):
            overlap_indexs = overlab_dict[element]
    ...
    @staticmethod
    def _make_overlab_dict(filelist):
        d = defaultdict(list)
